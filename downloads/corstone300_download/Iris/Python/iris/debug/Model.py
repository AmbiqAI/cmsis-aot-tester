...
class ModelBase(object):
    ...
    def __init__(self, client, verbose):
        ...
        # Get instance infos including properties for all targets.
        all_instance_infos = self.client.irisCall().instanceRegistry_getList(getProperties = True)
    ...
    def get_cpus(self):
        ...
        if not self.cpus:
            for info in self.instance_infos.values():
                if (info.properties.get('componentType') == 'Core') or \
                            (info.properties.get('executesSoftware', 0) != 0):
    ...
    def get_global_instance(self):
        """Returns global instance target.
        """
        if 0 not in self.targets:
            self.targets[0] = self._new_target([x for x in self.instance_infos.values() if x.instId == 0][0])
    ...
    def get_target_info(self):
        ...
        for info in self.instance_infos.values():
            if not info.instName.startswith("component."):
                continue
            yield TargetInfo(info.instName,
                             info.properties.get('componentName'),  # target_name
                             info.properties.get('version'),
                             info.properties.get('description'),
                             info.properties.get('componentType'),
                             info.properties.get('executesSoftware', 0) != 0,
                             info.properties.get('isHardwareModel', 0) != 0)

    def get_targets(self, prefix = 'component'):
        """Generator function to iterate all targets in the simulation"""
        for info in self.instance_infos.values():
            if (not prefix) or info.instName.startswith(prefix + "."):
                yield self.get_target(info.instName)
    ...
    def is_running(self):
        try:
            ...
            raise ValueError("Failed to get simulation time information")
    ...
    def _shutdown_model(self):
        # Delete event streams in model before shutting down the model.
        for target in self.targets.values():
            if isinstance(target, SyncTarget):
                target.destroy_event_buffer()
    ...
    def isConnected(self):
        """Returns whether the client is connected to the model"""
    ...
    def release(self, shutdown=False):
        ...
        if self.isConnected() == False:
            return  #client is already disconnected from the model
