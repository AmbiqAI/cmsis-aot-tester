...
class TargetEventSourceInfo(object):
    ...
    def to_event_source_info(cls, event):
        ...
        The event argument is modified and returned.

        The returned event and event.fields are plain Python dicts, modified and amended with some information and functions:
         - event.description is guaranteed to be present
         - event.get_field_info(field_name) returns the EventSozrceFieldInfo for a specific field.
         - event.fields[x].type is the Python type of the field.
         - event.fields[x].iris_type is the original Iris type (string) of the field.
         - event.fields[x].enums is a dict from integer value to symbol string.
         - event.fields[x].enums_array is the original Iris enums array (for descriptions etc).
         - event.fields[x].get_enum_symbol(int_value) returns the enum symbol string for an integer value.
         - event.fields[x].get_enum_info(int_value_or_symbol) returns the EnumElementInfo (as in enums_array).
         - event.fields[x].is_signed is True for signed ints.
         - event.fields[x].size is guaranteed to be present.
        ...
        for field in event.fields:
            ...
            field.size = field.get('size', 0)

            # Convert enums array to dict and preserve original enums array as enums_array.
            field.enums_array = field.get('enums', [])
            field.enums = {}
            for enum_info in field.enums_array:
                field.enums[enum_info.value] = enum_info.get('symbol', '')

            # Convert type to Python type and preserve original Iris type as iris_type
            field.iris_type = field.type
            if field.type == 'uint':
                if field.size == 0:
                    field.type = bytes
                else:
                ...
                field.type = str
            elif field.type == 'bytes': # Legacy ByteArray.
                field.type = bytes
            elif field.type == 'uint[]': # Legacy ByteArray.
                field.type = bytes
                ...
                warn('Unrecognised event field type "{}"'.format(field.type))

            # field = field: Force early binding since we define this function in a for loop.
            # Without this (late binding) field will always be bound to the last field in the loop.
            # See "Late Binding Closures" in https://docs.python-guide.org/writing/gotchas.
            def get_enum_symbol(int_value, field = field):
                """Get an enum string symbol for an integer value.
                Return "(0xabc)" for unknown integer 0xabc).
                """
                if int_value in field.enums:
                    return field.enums[int_value]
                return "(0x{:x})".format(int_value)

            def get_enum_info(int_value_or_symbol, field = field):
                """Get an EnumELementInfo for an integer value or string symbol.
                Return None if the enum is not found.
                """
                for enum in field.enums_array:
                    if (enum.value == int_value_or_symbol) or (enum.symbol == int_value_or_symbol):
                        return enum
                return None

            field.get_enum_symbol = get_enum_symbol
            field.get_enum_info   = get_enum_info

        def get_field_info(field_name):
            """Get EventSourceFieldInfo of an event field.
            """
            for field_info in event.fields:
                if field_info.name == field_name:
                    return field_info
            raise ValueError("Unknown field '{}' in get_field_info".format(field_name))

        event.description = event.get('description', '')
        event.get_field_info = get_field_info
...
class TargetBase(object):
    ...
    def __init__(self, instInfo, model):
        ...
        self.properties = instInfo.properties
        ...
        for group_info in group_info_list:
            ...
            iris.makeNamesHierarchical(resource_info_list, False)
    ...
    @property
    def memory_spaces_by_name(self):
        """Returns the memory spaces"""
        return self._memory_spaces_by_name
    ...
    def read_memory(self, address, memory_space = None, size = 1, count = 1, do_side_effects = False, attributes = {}):
        """
        :param address:
        ...
        :param attributes:
            Transaction attributes for the memory access.
            The attributes are represented as key/value pairs.
        ...
        result = iris.try_iris(self.client.irisCall().memory_read, None,
                               ...
                               address = address, byteWidth = size, count = count, attrib = attributes)
    ...
    def write_memory(self, address, data, memory_space = None, size = 1, count = None, do_side_effects = False, attributes = {}):
        """
        :param address:
            Address to begin reading from
        ...
        write_result = iris.try_iris(self.client.irisCall().memory_write, None,
                                     ...
                                     count = count, data = u64_array, attrib = attributes)
    ...
    def reg_get_bpt_support(self, reg_name):
        """
        Get string indicating supported breakpoint modes for ``reg_name``

        :param reg_name:
            The name of the register on which to query breakpoint functionality.
            The name can be in one of the following formats:

                * ``<group>.<register>``
                * ``<register>``

            The second form can only be used if the register name is unambiguous.
            Note that register fields never support breakpoints.

        Returns a string indicating ``reg_name`` breakpoint support, which
        will be one of:
            "r" : read only breakpoint support
            "w" : write only breakpoint support
            "rw": read and write breakpoint support
            ""  : no breakpoint support
            "?" : unknown
        """
        reg_info = self._get_register_by_name(reg_name).get("registerInfo", {})

        return reg_info.get("breakpointSupport", "?")
    ...
    def get_event_info(self, name=None):
        ...
        See to_event_source_info() for additional info and functions in the returned object
        (e.g. get_field_info(), get_enum_symbol() and get_enum_info()).
...
class AsyncTarget(TargetBase):
    ...
    def stdin(self):
        ...
        if not self._handle_semihost_io:
            raise ValueError("Not handling semihosting for target {}, try calling handle_semihost_io()"
                  .format(self.instance_name))
    ...
    def handle_semihost_io(self):
        ...
        try:
            ...
            esID = self.client.irisCall().eventStream_create(instId = self.instId,
                                                      evSrcId = event_src.evSrcId,
                                                      ecInstId = self.client.instId,
                                                      ecFunc = ecFunc)

            option = dict()
            option["useByteArray"] = False
            self.client.irisCall().eventStream_setOptions(instId = self.instId, esId = esID, options = option)
...
class SyncTarget(TargetBase):
    ...
    def destroy_event_buffer(self):
        """Destroy event buffer which contains all the events requested through add_event_callback().
        """
        self._event_manager.destroy_event_buffer()
