...
# String "__u8a".
STRING__u8a = 0x20006138755F5F05
...
class ByteArray(bytes):
    """Byte array container for Python 2 and Python 3.
    
    For Python 3 this is not strictly needed, but this offers str() and repr() which return
    a string which indicates that this is a ByteArray.
    
    For Python 2 this is only strictly needed when sending byte arrays, as normal Python 2 "bytes"
    are just "str"s and are sent as Iris Strings. To send a byte array use ByteArray(b"\0\1\2").
    
    (In Python 2 the "bytes" type is an alias for "str" and we use "str" as "ASCII text strings" in Python 2.
    Sending a byte array for every normal string would confuse the model. So for Python 2 we always send Python
    "str" as Iris String and this ByteArray type can be used to force ByteArray. In Python 3 there is a clear
    destiction between "str" (mapped onto Iris String) and "bytes" or "bytearray" (mapped onto Iris ByteArray)
    and for Python 3 this ByteArray class is identical to "bytes".)
    """
    def __str__(self):
        return "ByteArray(\"{}\")".format(self.decode("latin1").encode("unicode-escape").decode("latin1"))
...
def _encode_recursive(value, buffer):
    ...
    elif (isinstance(value, bytes) and not python2) or isinstance(value, ByteArray): # ByteArray
        u64len = (len(value) + 7) // 8
        u64jsonHasByteArray = False
        if not u64jsonHasByteArray:
            # Compatible version using an Object with a member __u8a:
            # Add Object prefix: Ultimately persist: {"__u8a": [0xcb00000000000009, 0x0807060504030201, 0x09]} for ByteArray [1,2,3,4,5,6,7,8,9].
            buffer.append(0xb000000000000000 + u64len + 5)
            buffer.append(1)
            buffer.append(STRING__u8a)
            buffer.append(0x8000000000000000 + u64len + 1)
        buffer.append(0xcb00000000000000 + len(value))
        _frombytes(buffer, value + (b"\0" * (u64len * 8 - len(value))))
...
def _u64array_to_bytes(buffer, offset, length):
    return ByteArray(b''.join(struct.pack('<Q', b) for b in buffer[offset:offset+((length + 7) >> 3)])[:length])
...
def _decode_recursive(buffer, offset=0):
    ...
    if msb4 in (0x0, 0xf): # +ve Number
        ...
        obj = {}
        offset += 2
        if (member_count == 1) and (buffer[offset] == STRING__u8a) and (buffer[offset + 1] == 0x8000000000000000 + container_len - 4) and (((buffer[offset + 2] + 7) & ~7) == 0xcb00000000000000 + (container_len - 5) * 8):
            # This is the compatible byte array encoding: A 0xcb byte array wrapped in an object with a __u8a member containing a NumberU64[].
            (value, elements_read) = _decode_recursive(buffer, offset + 2) # Decode actual byte array containted in the NumberU64[].
            return (value, elements_read + 4)
        ...
        elif msb8 == 0xcb: # Native ByteArray encoding. Also used recursively by the compatible __u8a object encoding.
