...
# \date   Copyright ARM Limited 2015-2023 All Rights Reserved.
...
if sys.version_info < (3,0):
    ...
    import queue as queue_namespace
    # In Python 3 str replaces unicode and unicode is removed.
    unicode = str
...
def safeStr(s):
    """Return an ASCII string (str) for any str, unicode, bytes or bytearray object using backslashed escape seqences for non-ASCII chars.
    """
    if isinstance(s, unicode):
        # Python 3 str (unicode string) and Python 2 unicode
        return s.encode("unicode-escape").decode("latin1")
    else: # Python 3 bytes/bytearray and Python 2 str
        # 1. convert bytes X into unicode code points X, 2. escape, 3. convert to str
        return s.decode("latin1").encode("unicode-escape").decode("latin1")
...
def compactStrInternal(value, ret, multiline, indentSingleLine, indentMultiLine, filterKeys):
    ...
    if isinstance(value, dict):
        ...
        elif isinstance(value, u64json.ByteArray):
            ret.append(indentSingleLine + ("ByteArray(\"{}\")".format(safeStr(value))))
        elif isinstance(value, unicode) or isinstance(value, str):
            ret.append(indentSingleLine + ("\"{}\"".format(safeStr(value))))
        else:
...
def nameToCname(name):
    """Return a valid C identifier derived from name accoring to the
...
class IrisPythonAdapter(object):
    ...
    def callIrisFunction(self, functionName, **kwargs):
        ...
        Return return value or raise error exception.
        """
        if self.verbose:
        ...
        if "result" in response:
            ...
            errorCode = response["error"].get("code", 0xeeee) # 0xeeee for missing code member.
            message = response["error"].get("message", "(no message)")
            data = response["error"].get("data", None)
            if self.verbose:
                print("R error={} ".format(error.IrisError(errorCode, response['error']['message'])) + compactStr(response["error"]))
            raise error.IrisError(errorCode, message, data)
        else:
            raise error.IrisError(error.E_malformatted_response, 'Received an invalid response', response)


    def instanceRegistry_getList(self, **kwargs):
        """instanceRegistry_getList() function which supports getProperties=True for old models which do not support getProperties=True
        by falling back to calling instance_getProperties() for each instance.
        """
        try:
            result = self.callIrisFunction("instanceRegistry_getList", **kwargs)
        except error.IrisError as e:
            if e.code == E_unsupported_argument_name:
                # Try again without getProperties.
                del kwargs["getProperties"]
                result = self.callIrisFunction("instanceRegistry_getList", **kwargs)
            raise e

        # Now result is the list of instances with or without properties.
        getProperties = kwargs.get("getProperties", False)
        if getProperties:
            # We requested properties. Make sure every InstanceInfo contains a properties member
            # to make all Python code relying on instanceRegistry_getList(getProperties=True) working,
            # especially model.get_cpus().
            # This transparently handles old models which do not understand getProperties=True
            # and it also handles instances gracefully which returned and empty/missing properties object.
            for instanceInfo in result:
                if ("properties" in instanceInfo) and (len(instanceInfo.properties) > 0):
                    continue
                try:
                    instanceInfo.properties = self.instance_getProperties(instId = instanceInfo.instId);
                except error.IrisError:
                    # Sigh. Even that failed. Synthesize the minimal instance properties.
                    instanceInfo.properties = {instId: instanceInfo.instId, instName: instanceInfo.instName}
...
class IrisClientBase(object):
    ...
    def _connect_handshake(self):
        ...
        try:
            ...
            if sys.version_info >= (3, 3) and e is ConnectionResetError: #ConnectionResetError exception is available since Python 3.3
                raise error.IrisError(error.E_not_connected, "Connection terminated during handshake.")
            elif e is socket.timeout:
                raise error.IrisError(error.E_not_connected, "Connection timeout during handshake.")
            else:
                raise e
    ...
    def _connect_epilog(self):
        ...
        try:
            ...
            raise error.IrisError(error.E_not_connected, "Failed to register instance.")
    ...
    def irisCall(self):
        ...
        These Iris functions return the return value or raise an error exception.
        """
        ...
        with self.receivedResponsesCondVar:
            while requestId not in self.receivedResponses:
                ...
                if sys.version_info < (3, 0):
                    ...
                    self.receivedResponsesCondVar.wait_for(lambda: (self.receivedResponsesSuccess or (not self.isConnected_)), timeout=self.timeoutInMs/1000.0)
                if not self.isConnected_:
                    raise error.IrisError(error.E_not_connected, "Connection is closed.")
    ...
    def processReceivedRequests(self):
        ...
        while not self.receivedRequests.empty():
            ...
            try:
                ...
                # make sure to mark all requests we processed as done, even if
                # it caused an error
                self.receivedRequests.task_done()
...
class IrisUnixDomainClient(IrisClientBase):
    ...
    def ConnectToSocket(
        ...
        error.
        """
        client = IrisUnixDomainClient(client_name, IrisInstance(), verbose, timeoutInMs=timeout)
        client.connect(sock)
        return client
...
def makeNamesHierarchical(resourceInfos, addMembers = True):
    ...
    # pointers, but it also solves the problem that we do not know the order in
...
def jsonIntegerObjectWarning(obj):
    if '__Iris64L' in obj:
        print("warning: received integer encoded as object; this is not supported by the Iris Python library: {}".format(obj))
